...
FRONTEND_BUILD_DIR = os.path.join(PROJECT_ROOT, "ui", "build")
...
def _api_status_payload():
    """Return a consistent API status payload."""
    return {
        'status': 'ok',
...
def _frontend_build_exists() -> bool:
    """Return True when a compiled React frontend is available."""
    return os.path.exists(FRONTEND_INDEX_FILE)


def _resolved_upload_dir() -> str:
...
def _save_request_files(files) -> tuple[list[str], list[str]]:
    """Persist uploaded files and return saved paths plus any unsupported filenames."""
    ...
    return saved_paths, unsupported_files
...
def health_check():
    ...
    """
    return jsonify(_api_status_payload())
...
def index(path: str):
    """
    Serve the compiled frontend when available, otherwise return API status.
    """
    if path == 'api':
        return jsonify(_api_status_payload())
    ...
    if _frontend_build_exists():
        requested_asset = os.path.join(FRONTEND_BUILD_DIR, path)
        if path and os.path.isfile(requested_asset):
...
def initialize_agent():
    ...
    model_name = data.get('model_name') or os.getenv('OPENAI_MODEL_NAME') or 'gemma3:4b'
    ...
    try:
        ...
        if (not database_url and not use_mock_mongo) or not vector_store_dir:
            raise EnvironmentError(
                "Required environment variables DATABASE_URL and VECTOR_STORE_DIR are not set. "
...
def update_case():
    ...
    try:
        case = agent.database_manager.update_case(case_id, **data)
        if not case:
            return jsonify({
                'success': False, 
...
def update_client():
    ...
    try:
        client = agent.database_manager.update_client(client_id, **data)
        if not client:
            return jsonify({
                'success': False, 
                'message': f'Client with ID "{client_id}" not found'
...
def ask_question():
    ...
    try:
        ...
        if not chat_interaction:
            return jsonify({
                'success': False, 
                'message': 'The assistant did not return an answer. Re-initialize the model and try again.'
            }), 500
...
def upload_document():
    ...
    This endpoint accepts uploaded files and optionally local file paths or
    web URLs for backward compatibility. Uploaded files are stored in the
    configured upload directory before indexing.
    
    Request Body:
        files (multipart): Uploaded files such as PDF, DOC, DOCX, or TXT
    ...
    if uploaded_files:
        file_paths, unsupported_files = _save_request_files(uploaded_files)
    else:
    ...
    if not file_paths and not urls:
        return jsonify({
            'success': False, 
            'message': 'Upload at least one PDF, DOC, DOCX, or TXT file'
        }), 400
        ...
        if result <= 0:
            return jsonify({
                ...
                'message': (
                    ...
                    'PDF, DOCX, and TXT are fully supported. '
                    'For legacy DOC files, save them as DOCX or TXT and try again.'
                )
            }), 400
        
        return jsonify({
            'success': True,
            'message': f'Successfully uploaded and indexed {result} document(s)',
