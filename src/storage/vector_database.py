...
def extract_text_from_docx(file_path):
    """
    Extract text from a DOCX file using the standard library.

    DOCX files are ZIP archives containing WordprocessingML XML.
    """
    paragraphs = []
    ...
    try:
        ...
        for element in root.iter():
            if element.tag.endswith(namespace_suffix) and element.text:
                current_paragraph.append(element.text)
            elif element.tag.endswith(paragraph_suffix) and current_paragraph:
        ...
        logger.info(f"DOCX extraction failed for {file_path}: {e}")
        return ""
...
def extract_text_from_doc(file_path):
    ...
    Legacy `.doc` is a binary format. Without an external parser installed,
    we fall back to extracting readable text runs from the binary payload.
    """
    try:
        with open(file_path, "rb") as f:
            payload = f.read()
    except Exception as e:
        logger.info(f"DOC extraction failed for {file_path}: {e}")
        return ""
    ...
    utf16_chunks = [
        ...
        for chunk in re.findall(rb"(?:[\x20-\x7E]\x00){4,}", payload)
    ]

    combined_chunks = []
    seen_chunks = set()
    for chunk in ascii_chunks + utf16_chunks:
        normalized = " ".join(chunk.split())
        if len(normalized) < 4 or normalized in seen_chunks:
            continue
        seen_chunks.add(normalized)
        combined_chunks.append(normalized)
...
def load_source_with_fallback(source):
    ...
    - DOCX files
    - DOC files (best-effort extraction)
    ...
    if ext == ".pdf":
        ...
        docx_text = extract_text_from_docx(source)
        if docx_text.strip():
            doc = Document(page_content=docx_text, metadata={'source': source})
    elif ext == ".doc":
        doc_text = extract_text_from_doc(source)
        if doc_text.strip():
            doc = Document(page_content=doc_text, metadata={'source': source, "legacy_doc_extraction": True})
    elif ext in [".png", ".jpg", ".jpeg", ".bmp", ".tiff"]:
        ocr_text = extract_text_with_ocr(source)
        if ocr_text.strip():
            doc = Document(page_content=ocr_text, metadata={'source': source, "ocr_applied": True})
...
class VectorStoreManager:
    ...
    def __init__(self, file_path=None, nlist=100, gpu_memory_mb=128):
        ...
        self.gpu_res = None
        self.batch_size = 128
        self.embedding_model_name = os.getenv("EMBEDDING_MODEL", "openai/text-embedding-3-small")
        ...
        if FAISS_GPU_AVAILABLE and StandardGpuResources is not None:
            try:
                ...
                logger.info(f"GPU resources unavailable, continuing with CPU FAISS: {e}")
                self.gpu_res = None
        else:
            logger.info("FAISS GPU support not available, continuing with CPU FAISS.")
        ...
        # Use the standard recursive splitter to avoid first-run tokenizer downloads.
    ...
    def _detect_embedding_dimension(self):
        """Resolve embedding dimension without unnecessary API calls when possible."""
        normalized_model = self.embedding_model_name.strip().lower()
        known_dimension = EMBEDDING_DIMENSIONS.get(normalized_model)
        if known_dimension:
