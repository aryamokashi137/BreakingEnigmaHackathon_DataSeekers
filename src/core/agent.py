...
class QuestionProcessingError(RuntimeError):
    """Raised when a question cannot be processed in a user-actionable way."""

    def __init__(self, user_message: str, *, status_code: int = 500, cause: Exception | None = None):
...
class LegalRagAgent:
    ...
    def __init__(self, database_manager: DatabaseManager, model_name: str = None):
        """
        ...
        self.graph = None
        model_name = model_name or os.getenv("OPENAI_MODEL_NAME") or "gemma3:4b"
    ...
    def _rebuild_rag_pipeline(self) -> None:
        """Rebuild the RAG pipeline so it always uses the current model and database state."""
        self.rag_pipeline = RAGPipeline(llm=self.llm, database_manager=self.database_manager)
        self.graph = build_workflow(self.rag_pipeline)

    def _build_processing_error(self, error: Exception) -> QuestionProcessingError:
        """Convert low-level model/provider failures into actionable user-facing errors."""
        error_text = str(error).strip() or error.__class__.__name__
        normalized_error = error_text.lower()

        if any(marker in normalized_error for marker in ("connection error", "timed out", "timeout", "temporarily unavailable")):
            return QuestionProcessingError(
                (
                    f"Could not reach the configured AI model '{self.model_name}'. "
                    "Check your internet connection and the provider settings in `.env`, "
                    "or switch to a local Ollama model such as `gemma3:4b`."
                ),
                status_code=503,
                cause=error,
            )

        if any(marker in normalized_error for marker in ("api key", "authentication", "unauthorized", "invalid_api_key", "incorrect api key")):
            return QuestionProcessingError(
                (
                    f"The configured AI model '{self.model_name}' could not authenticate. "
                    "Check the API key and base URL in `.env`."
                ),
                status_code=502,
                cause=error,
            )

        if ("model" in normalized_error and "not found" in normalized_error) or "not supported" in normalized_error:
            return QuestionProcessingError(
                (
                    f"The model '{self.model_name}' is not available with the current provider. "
                    "Choose a supported model name and re-initialize the assistant."
                ),
                status_code=400,
                cause=error,
            )
        ...
        return QuestionProcessingError(
            f"Error processing question with model '{self.model_name}': {error_text}",
            status_code=500,
            cause=error,
        )
    ...
    def upload_documents(self, web_urls: List[str] = None, doc_files: List[str] = None) -> int:
        ...
        try:
            ...
            raise self._build_processing_error(e) from e
    ...
    def ask_rag_question(self, question: str, uuids_for_retrieval: List[str] = None, 
        ...
        try:
            ...
            state = {
                ...
                "included_web_pages": included_web_pages,
            }
        ...
        except QuestionProcessingError:
            raise
        except Exception as e:
