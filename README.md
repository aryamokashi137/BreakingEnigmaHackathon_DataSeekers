...
5. **Optional: start backend and frontend together from the project root**
...
# If ui/build exists, the frontend will also be served at http://localhost:5000
```
...
# API requests will proxy to /api
```
...
Legal RAG Pipeline/
├── app_data/
│   ├── metadata handled by configured MongoDB database
|   |
...
- **Case-Isolated Storage**: Each case maintains separate MongoDB metadata records and vector stores
- **MongoDB Metadata Store**: Clients, cases, chat history, and document metadata are stored in MongoDB
...
- **Vector Store Coordination**: Seamlessly integrates MongoDB metadata with FAISS vector operations
...
- **sql_classes.py**: Compatibility dataclasses that preserve the application's case/client/document object model while MongoDB handles persistence
